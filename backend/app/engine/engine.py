...
class RuleEngine:
    ...
    def run(self, df: pd.DataFrame, workflow: dict) -> dict:
        ...
        for idx, step in enumerate(workflow["steps"]):
            try:
                ...
                context.log("error", f"Step {idx} ({rule_type}) failed: {str(e)}", 0)
    ...
    def preview(self, df: pd.DataFrame, workflow: dict, max_rows: int = 20) -> dict:
        ...
        if result["outputs"]:
            ...
            after_data["rows"] = (
                result["outputs"][first_sheet].head(max_rows).to_dict(orient="records")
            )
        else:
            # No outputs created, show filtered current_df (not available after execution)
            after_data["rows"] = []

        return {"before": before, "after": after_data, "logs": result["logs"]}
